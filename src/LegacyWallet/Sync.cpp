                           ...
                           bool fetchTimestamp)
{
    ...
    if (fetchTimestamp)
    {
        ...
        /* Couldn't get timestamp, maybe old node or coinlegacyd closed */
        if (blockTime != "")
        {
...
void saveCSV(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node)
{
    ...
    for (size_t i = 0; i < numTransactions; i++)
    {
        ...
        if (t.totalAmount != 0) {
            ...
            if (t.totalAmount < 0)
            {
                /* Put CLY in separate field, makes output more usable in spreadsheet */
                std::string splitAmtCLY = formatAmount(-t.totalAmount);
                boost::replace_all(splitAmtCLY, " ", ",");
                myfile << "-" << splitAmtCLY << ",OUT\n";
            }
            else
            {
                std::string splitAmtCLY = formatAmount(t.totalAmount);
                boost::replace_all(splitAmtCLY, " ", ",");
                myfile << splitAmtCLY << ",IN\n";
            }
        }
    }
    myfile.close();
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    {
        std::cout << "Your Coinlegacyd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting Coinlegacyd.";
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[CLY " + shortName + "]: ");
}
