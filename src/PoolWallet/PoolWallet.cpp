...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 10 shells = 0.1 CLY */
  if (cmd.fee == 10) {
    feeString = "0.1 CLY (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " CLY";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " CLY, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " CLY" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string cly = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << cly << " CLY belongs to you!";
    }
  }
  ...
  {
    std::string cly = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << cly << " CLY were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Coinlegacy v" << PROJECT_VERSION << " Poolwallet" << std::endl;
  ...
  std::cout << "Please note that usage of simplewallet/poolwallet has been "
            ...
            << "https://github.com/coinlegacy/turtle-pool/pull/5" << std::endl;
